...
class Setting {
  ...
  */
  ...
  async getSettingType(settingType) {
    debug('Adding an setting to the database')
   
    const { resource } = await this.container.item(settingType).read()
    return resource
  }
  ...
  async updateSetting(settingId, rep) {
    ...
    const doc = merge.all([id, typeObject, rep]);    
    const {resource : updated} = await this.container.item(settingId).replace(doc)
